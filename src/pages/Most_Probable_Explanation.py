...
def layout_mpe():
    ...
    return dbc.Container(
        [
            dcc.Store(id='memory_mpe', data={'time_list': [c.get_default_dic_mpe()], 'time': 0}),
            dbc.Row(
                [
...
def evid_gen(time, n1, n2, n3, n4, n5, dd_vals, b_e, op_s, e_var, e_in, q_var, e_op, op_i, mem, t_max):
    """
...
# )
def erg_controller(t, cb, e_var, e_in, mem_now):
    """
    ...
    """
    maxima =  None if mem_now.get('maxima') is None else jpt.variables.VariableMap.from_json(variables=iter(c.in_use_tree._variables), d=mem_now.get('maxima')[mem_now.get('page')])
    page = mem_now.get('page')
    likelihood = mem_now.get('likelihood')
    if cb == "b_erg_pre_mpe":
        page -= 1
        if page == 0:
            mem_now.update({'erg_b': (True, False), 'page': page})
            return mpe(maxima[page], likelihood), mem_now
        else:
            mem_now.update({'erg_b': (False, False), 'page': page})
            return mpe(maxima[page], likelihood), mem_now
        ...
        if len(maxima) > page + 1:
            ...
            mem_now.update({'erg_b': (False, True), 'page': page})
            return mpe(maxima[page], likelihood), mem_now
    else:
        page = 0
        ...
        try:
            ...
            mem_now.update({'maxima': maxima_li, 'likelihood': likelihood})

        except Exception as e:
            mem_now.update({'erg_b': (True, True), 'page': page})
            print("Error was", type(e), e)
            return [html.Div("Unsatisfiable", className="fs-1 text text-center pt-3 ")], mem_now
        if len(maxima) > 1:
            mem_now.update({'erg_b': (True, False), 'page': page})
            return mpe(maxima[0], likelihood), mem_now
        else:
            mem_now.update({'erg_b': (True, True), 'page': page})
            return mpe(maxima[0], likelihood), mem_now
...
def update_time(time, e_var, e_in, e_op, q_var, mem):
    ...
    t = c.create_prefix_text_mpe(len_fac_q=len(f_val.get("e_var")), len_fac_e=len(f_val.get("e_var")))
    maxima = None
    maxima: List[jpt.variables.LabelAssignment]
    if f_val.get('maxima') is not None:
        maxima: List = []
        maxim: dict["str", any]
        for maxim in f_val.get('maxima'):
            maxima_li = jpt.variables.LabelAssignment.from_json(variables=iter(c.in_use_tree._variables), d=maxim)
            maxima.append(maxima_li)
    page = f_val.get('page')
    likelihood = f_val.get('likelihood')
    erg = [] if maxima is None else mpe(maxima[page], likelihood)
    b_erg = f_val.get('erg_b')
...
def button_time(cb, value, max_time, mem ):
    ...
    if cb is not None and cb == "mpe_timer_plus":
        new_dic = c.get_default_dic_mpe().copy()
